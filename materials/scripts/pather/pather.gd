@tool
class_name ControlablePathFollow2D
...
@export_range(0.001,99,0.001,"or_greater","hide_slider","suffix:px") 
var speed: float = 200.0
@export_range(0,99,0.0001,"or_greater","hide_slider","suffix:px") 
...
@export_range(-180,180,0.001,"suffix:deg") 
var to_rotation: float = 0
...
func _physics_process(delta: float) -> void:
	...
	if enabled and not blocked:
		...
		rotation_degrees = move_toward(rotation_degrees, to_rotation, rotation_speed * delta)
...
func to_rotation_deg(_rotation: float = 0.0) -> void:
	to_rotation = wrapf(_rotation, -180.0, 180.0)
